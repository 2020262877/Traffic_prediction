...
def main():
    ...
    for i in range(0, 5000):
        if tree_expansion(ws, uav, d_obs, s_obs, 1, 0.4):
...
def tree_expansion(ws, uav, obs, s_obs, step=1, threshold=0.4):
    ...
    ws.bounds = [
        (n_last.x, n_last.y),
        (ws.bounds[1][0], n_last.y),
        ws.bounds[2],
        (n_last.x, ws.bounds[3][1])
    ]
    ...
    # if not cons.scope_constrain():
    #     x = math.floor(uav.tree[-1][0].x) - 0.5
    #     y = math.floor(uav.tree[-1][0].y) - 0.5
    #     # print('(x,y):({},{})'.format(x, y))
    #     n_new = State(x, y, 0)
    #     uav.tree[-1] = (n_new, uav.tree[-1][1])
    #
    ...
    #     print('state0123:0--{},1--{},2--{},3--{}'.format(state['0'], state['1'], state['2'], state['3']))
    #     action = state.idxmax()
    #     print('idxdmax:{}'.format(action))
    #     if action == '0':
    #         xx, yy = x, y + 1
    #     elif action == '1':
    #         xx, yy = x, y - 1
    ...
    #
    # for i in range(len(uav.tree)):
    #     print('point{}:({},{})'.format(i, uav.tree[i][0].x, uav.tree[i][0].y))
    # return n_new.return_distance(uav.end) <= 1
...
class Constrains:
    def __init__(self, dyn_obs_s_set, sta_obs_s_set, s, n_near, step=1, threshold=0.4):
