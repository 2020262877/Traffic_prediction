...
def main():
    ...
    obs_state2 = DynamicObstacleState(14, 14, 'black', 1)
    ...
    time = (datetime.now() - start_time).microseconds
    print('cost (distance): {}'.format(cost))
    # print('path planning time: {}'.format(time))
    print('path planning time(microseconds):{} '.format(time))
    # return cost, time
    d_obs.plot()
    s_obs.plot()
    uav.plot_tree()
...
class Constrains:
    ...
    def scope_constrain(self):
        ...
        for dyn_obs_s in self.dyn_obs_s_set:
            x = dyn_obs_s.x
            y = dyn_obs_s.y
            is_qualify = sqrt((x - self.s.x) ** 2 + (y - self.s.y) ** 2) > 2
            if not is_qualify:
                return False
...
if __name__ == "__main__":
    ...
    #     sum_time += time
    #     sum_cost += cost
    # print('Avg_cost:{}\nAvg_time(milliseconds):{}'.format(sum_cost / 50, sum_time / 50 / 1000))
